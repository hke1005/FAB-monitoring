...
if df_raw.empty:
    ...
    if df.empty:
        ...
        with main_col:
            # HTML Flexbox를 사용하여 제목은 왼쪽, ISO 텍스트는 오른쪽 아래에 배치 (추가 요청분 완벽 반영)
            st.markdown("""
                ...
                with alert_container:
                    ...
                    for lot_num in unique_lots:
                        ...
                        if len(anomalies) == 1:
                            ...
                            with a_col2:
                                st.markdown(f"<div style='font-weight: bold; font-size: 13px; margin-bottom: 2px; color: #374151;'>공정 변 이상 감지</div><div style='font-size: 12px; color: #6B7280;'>배치: <b>{lot_num}</b> | {param_name}: <span style='color:#EF4444; font-weight:bold;'>{val_str} {unit}</span></div>", unsafe_allow_html=True)
                            with a_col3:
